...
O projeto é desenvolvido por especificação. A baseline vigente é a SDD-MICROHIL 0.29.4. O HOST já inclui executor FMI, sessão de execução compartilhada pela CLI e pela base Qt, logging binário, perfil YAML e comunicação host. O firmware ESP32 teve estados, aquisição, READ_ACK, watchdog e loopbacks de atuação verificados fisicamente. O Play HiL da GUI completou 500 passos com Agent, DAQC e FMU de três loopbacks; precisão elétrica, duty PWM intermediário, qualificação temporal e Raspberry Pi permanecem pendentes. Comece pelo [índice dos documentos](docs/README.md), pela [especificação de trabalho](docs/spec.md) e pelas [decisões](docs/decisions.md). As regras vigentes para comentários, nomenclatura e formatação estão em [AGENTS.md](AGENTS.md) e na [constituição](docs/constitution.md).
