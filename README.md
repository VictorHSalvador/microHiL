...
O projeto é desenvolvido por especificação. A baseline vigente é a SDD-MICROHIL 0.29.2. O HOST já inclui executor FMI, sessão de execução compartilhada pela CLI e pela base Qt, logging binário, perfil YAML e comunicação host. O firmware ESP32 teve estados, aquisição, READ_ACK, watchdog e loopbacks de atuação verificados fisicamente. O runner C completou 200 passos com Agent, DAQC e FMU e o log mostrou a evolução do loopback analógico; precisão elétrica, qualificação temporal e Raspberry Pi permanecem pendentes. Comece pelo [índice dos documentos](docs/README.md), pela [especificação de trabalho](docs/spec.md) e pelas [decisões](docs/decisions.md). As regras vigentes para comentários, nomenclatura e formatação estão em [AGENTS.md](AGENTS.md) e na [constituição](docs/constitution.md).
...
Aplicação C modular para Linux destinada a executar FMU FMI 2.0 Co-Simulation, com thread de simulação, logging binário assíncrono e plotagem separada por gnuplot. Uma execução física completou preflight, sessão XRCE/ROS, STREAMING, 200 passos FMI e DISABLE com `SCHED_FIFO` ativo; o log mostrou a evolução do loopback GPIO25→GPIO32. Esse resultado não mede precisão elétrica nem qualifica desempenho contínuo em tempo real.
