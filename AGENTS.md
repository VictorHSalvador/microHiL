...
Revisão 0.21: usar ADR-003/ICD para o reuso RaspDAQ. Base little-endian, SYNC 59 72, STATUS uint8 só CONFIG, DATA com SEQ e N fixo. O coordenador C é o único dono de `/dev/ttyUSB*` pela CH340; não usar libusb direto nem iniciar um agente serial concorrente. Não usar CRC, retransmissão, replay ou sessão no fio; em perda, contabilizar quando detectável e seguir para o pacote novo. READ_ACK MID 03 comprova leitura sem pedir reenvio; XRCE MID 04 alcança o Agent somente por ponte UDP local do coordenador. Não atribuir essas extensões à referência nem afirmar validação sem execução.
