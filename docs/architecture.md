...
O controlador HOST de estados aguarda confirmação somente fora da thread de simulação. CONFIG UART usa `daqc_config_timeout_ms`, inicialmente 10 ms. Depois de CONFIG ENABLE, a espera de estabilização `daqc_ros_startup_delay_ms`, inicialmente 6.000 ms e configurável, antecede a publicação de `DaqcSetup`; ela reproduz o procedimento físico validado com Agent/XRCE e não confirma setup. A confirmação ROS de `DaqcSetup` usa `daqc_ros_timeout_ms`, inicialmente 100 ms. Play HiL executa o preflight SCHED_FIFO, solicita CONFIG ENABLE, aguarda a estabilização, publica `DaqcSetup` e exige `DaqcState` ENABLE com perfil/configuração aplicados antes de solicitar STREAMING e criar a thread FMI. Essas esperas são prévias à thread FMI. Stop, término e Error solicitam DISABLE após congelar a produção; a confirmação ausente é reportada, sem alterar o relógio do modelo.
