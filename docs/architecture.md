...
A USB-C da placa liga host → CH340 → UART do ESP32; não é uma interface nativa USB de ESP32-S3. O coordenador C usa exclusivamente `/dev/ttyUSB*`, configurado pelo driver CH341 Linux. O worker `daq_serial_service` implementa RX/TX limitado em thread própria no HOST. O coordenador demultiplexa XRCE recebido e preserva respostas Agent→DAQC em FIFO de 64 datagramas de até 128 bytes; CONFIG não limpa essa fila independente. A ponte UDP foi verificada primeiro com socket local e depois no [ciclo físico runner–Agent–DAQC–FMU](evidence/host-runner-daqc-fmu-2026-09-14.md).
...
O controlador HOST de estados aguarda confirmação somente fora da thread de simulação. CONFIG UART usa `daqc_config_timeout_ms`, inicialmente 10 ms. Depois de CONFIG ENABLE, a espera de estabilização `daqc_ros_startup_delay_ms`, inicialmente 6.000 ms e configurável, antecede a publicação de `DaqcSetup`; ela reproduz o procedimento físico validado com Agent/XRCE e não confirma setup. A confirmação ROS de `DaqcSetup` usa `daqc_ros_timeout_ms`, inicialmente 100 ms. Play HiL executa o preflight SCHED_FIFO, abre a TTY como dono exclusivo com DTR/RTS inativos e, por padrão configurável, reseta a DAQC por RTS por 100 ms e aguarda 1.000 ms. Em seguida descarta RX pendente, exige uma confirmação CONFIG DISABLE nova, solicita CONFIG ENABLE, aguarda a estabilização, publica `DaqcSetup` e exige `DaqcState` ENABLE com perfil/configuração aplicados antes de solicitar STREAMING e criar a thread FMI. Essas esperas são prévias à thread FMI. Stop, término e Error solicitam DISABLE após congelar a produção; a confirmação ausente é reportada, sem alterar o relógio do modelo. Uma execução física curta completou esse ciclo e 20 passos FMI; ela não qualifica o loopback elétrico ou o tempo real.
...
A implementação inicial fixa aquisição/atuação no núcleo 1 com prioridade 9, comunicação no núcleo 0 com prioridade 8 e `daqc_ros` no núcleo 0 com prioridade 4 e pilha de 6144 bytes. A supervisora ROS também fica no núcleo 0, com prioridade 3 e pilha de 8192 bytes; após a tarefa UART estar criada, ela é a única chamadora de `DaqcRosStart` e tenta somente fora de STREAMING. Assim `app_main`, configurada com pilha de 4096 bytes, não executa a inicialização micro-ROS. A comunicação copia cada DATA host→DAQC para um snapshot de atuação limitado; a tarefa I/O consome somente o snapshot mais recente na grade de aquisição e é a autora normal de GPIO/DAC/PWM durante STREAMING. Um snapshot inválido gera diagnóstico assíncrono e não mantém o parser preso; transições para fora de STREAMING descartam snapshot pendente. A tarefa de I/O acorda pela grade configurada em `acquisition.frequency_hz`, coleta e transmite somente o snapshot atual durante STREAMING; 100 Hz é o valor inicial e 1…100 Hz é o limite de admissão de software. Esses são parâmetros de implementação para manter ROS abaixo do tráfego crítico, não evidência de orçamento, ausência de inversão de prioridade ou cumprimento de deadline; a qualificação deve medi-los no alvo.

Na comunicação, `uart_read_bytes` é consultada sem bloqueio. Sem bytes, a tarefa arma um `esp_timer` one-shot de 1 ms e bloqueia na própria notificação; o callback do timer somente notifica a tarefa. Isso evita que uma conversão de espera de 5 ms para zero ticks monopolize o núcleo 0 e impeça a criação/inicialização do cliente ROS. Um prazo normal é medido pelo relógio monotônico do timer e não altera a frequência de aquisição, os contratos de transferência de até 5 ms, os MIDs nem a prioridade de CONFIG. Se o timer não puder ser armado, a tarefa espera um tick como estado degradado para não executar busy loop; essa exceção pode aumentar a latência e deve ser registrada e medida antes de qualquer alegação temporal.
