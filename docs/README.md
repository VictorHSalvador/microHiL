...
Baseline vigente: **SDD-MICROHIL 0.26.17**. O histórico central está em [sdd-versions.md](sdd-versions.md); revisões internas preservadas nos documentos continuam úteis, mas não substituem esse registro. A verificação estrututal reproduzível está em [sdd-versioning.json](../.spec/verification/sdd-versioning.json).
...
Revisão 0.26.16: o runner terminal encerra ao receber EOF em vez de reapresentar o menu indefinidamente. Revisão 0.26.17: o artefato ROS do runner declara RPATH para as bibliotecas Humble e das interfaces, permitindo o uso de `CAP_SYS_NICE` sem depender de `LD_LIBRARY_PATH`. Ambas são correções HOST; o ensaio integrado DAQC–coordenador–FMU ainda está pendente.
