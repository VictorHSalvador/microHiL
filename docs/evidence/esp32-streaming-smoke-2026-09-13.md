...
2. **Ciclo integrado com configuração e STREAMING:**
   ...
   - **Encerramento seguro (REQ-F-07):** envio de `CONFIG DISABLE` (`59 72 01 01`) resultou em confirmação imediata `59 72 01 01 01`, cessando o streaming e aplicando nível seguro (zero) nas saídas físicas.

## Limitações

O ensaio foi executado com o harness Python sobre a UART física e ponte UDP. A atuação de dados host→DAQC de 21 bytes não foi exercitada neste ensaio. Os canais analógicos e digitais leram sinais de pinos flutuantes/pulls internos, sem instrumento de bancada para calibração de precisão elétrica. A integração completa com o coordenador C (`fmu_rt_runner`) permanece como próximo passo.
