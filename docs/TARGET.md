...
A aquisição AI/DI usa `acquisition.frequency_hz` no YAML e em `DaqcSetup`. O valor inicial de novos perfis é 100 Hz e o firmware aceita 1…100 Hz antes de STREAMING. A tarefa de I/O coleta o snapshot nessa cadência e não cria backlog: a comunicação e a FMU trabalham com a atualização mais recente disponível. O teto atual decorre do tick FreeRTOS de 10 ms configurado no firmware, que permite uma tarefa periódica sem polling até 100 Hz; não é limite elétrico nem medição de UART. Validar taxa efetiva e jitter em bancada.
