...
A aquisição AI/DI usa `acquisition.frequency_hz` no YAML e em `DaqcSetup`. O valor inicial de novos perfis é 100 Hz e o firmware aceita 1…400 Hz antes de STREAMING. A tarefa de I/O coleta o snapshot nessa cadência e não cria backlog: a comunicação e a FMU trabalham com a atualização mais recente disponível. O teto atual decorre do orçamento nominal UART do frame de aquisição de 33 bytes, não da capacidade elétrica do ESP32; validar taxa efetiva e jitter em bancada.
