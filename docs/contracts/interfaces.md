...
**U:** placa informada usa CH340/CH341, VID:PID 1a86:7523, conectando USB do host à UART do ESP32. Os VID/PID 1d6b:0104 e caminhos ep0/ep1/ep2 do RaspDAQ pertencem ao gadget Linux, não à placa atual. O coordenador abre um único `/dev/ttyUSB*`, configura 8N1/baud aprovado e preserva a política de reset fora de STREAMING. Na abertura, DTR e RTS ficam inativos. O preflight pode executar o reset configurável aprovado: DTR inativo, RTS ativo por 100 ms, RTS inativo e estabilização por 1.000 ms antes de descartar RX e confirmar DISABLE; o valor inicial dessa opção é habilitado. Não abrir um segundo processo, agente serial ou cliente libusb para consumir os mesmos bytes.
...
CONFIG 01/02/03 altera somente o estado da DAQC. A seleção do perfil usa `DaqcSetup.profile_id` pelo MID 04 em DISABLE ou ENABLE, nunca dentro do ciclo. O identificador seleciona um perfil compilado no firmware; `DaqcState.profile_id` e `profile_applied` confirmam sua aplicação. O arquivo de configuração HOST contém o mapeamento entre FMU e o perfil selecionado e deve ser validado integralmente antes do Play. `DaqcSetup` transporta somente a configuração ADC/PWM aprovada; não transporta descritores nem mapa de GPIOs variável. STREAMING só é aceito quando o perfil selecionado e o schema do host são compatíveis. O perfil ESP32 usa `profile_id = 1` e layout fixo de 28 bytes de aquisição e 21 bytes de atuação. O RaspDAQ usa schemas compilados e não fornece essa seleção micro-ROS.

Na DAQC, a tarefa de comunicação copia DATA host→DAQC para mailbox de atuação de capacidade um; a tarefa de I/O no núcleo 1 consome o último snapshot disponível na sua próxima ativação durante STREAMING. Assim a comunicação não escreve GPIO/DAC/PWM diretamente e frames antigos não formam backlog. Um DATA inválido é descartado após diagnóstico assíncrono, sem impedir a busca de frames posteriores; transições para ENABLE ou DISABLE invalidam o snapshot pendente.
...
|---|---|
| Inicialização do serviço/firmware | Host abre a TTY com DTR/RTS inativos; se configurado, reseta a DAQC por RTS e aguarda 1.000 ms; depois descarta RX anterior, confirma CONFIG DISABLE novo, permanece sem DATA e mantém recepção CONFIG ativa |
...
Antes do CONFIG ENABLE→STREAMING, o controlador HOST confirma CONFIG ENABLE, aguarda a estabilização configurável `daqc_ros_startup_delay_ms` (inicialmente 6.000 ms) e então publica `DaqcSetup` com `command=ENABLE`, `profile_id`, `apply_configuration=1` e a configuração ADC/PWM validada. A estabilização reproduz o procedimento físico do Agent/XRCE, mas não é confirmação. Depois da publicação, ele aguarda `DaqcState` com ENABLE efetivo, `profile_applied=1` e `configuration_applied=1`, usando `daqc_ros_timeout_ms` configurável, inicializado em 100 ms. A recepção ocorre em thread ROS própria; essas esperas antecedem a thread de simulação e não alteram o relógio FMI. O setup de estado STREAMING pode ser publicado para observação depois da confirmação CONFIG, mas não participa da autorização de atuação.
...
TX organiza três classes: (1) encerramento/CONFIG e READ_ACK; (2) última atualização DATA por direção; (3) tráfego XRCE limitado. Encerramento tem entrada reservada e invalida DATA pendente da execução. Uma atualização DATA ainda não enviada pode ser substituída pela mais recente, contando coalescência; nunca manter fila crescente de atuação antiga. Não cortar/intercalar bytes dentro de um quadro já iniciado. READ_ACK também é coalescido para a maior sequência lida dentro do STREAMING atual. XRCE usa uma FIFO HOST limitada a 64 datagramas de até 128 bytes, preserva ordem e é servido depois do tráfego crítico; saturação incrementa diagnóstico e é reportada ao produtor sem substituir silenciosamente uma resposta anterior. Confirmação CONFIG não limpa essa FIFO porque a sessão Agent–cliente é independente da máquina de estados CONFIG.

DATA usa mailbox de capacidade um: produtor substitui valor ainda não consumido e incrementa contador de coalescência. Não há backlog, replay ou retransmissão. A fila XRCE tem profundidade 64 no HOST; esse limite técnico ainda precisa de qualificação sob carga e não altera o MTU de 128 bytes. Os 60 s são prazo de desativação, nunca capacidade de armazenamento. RX/CONFIG deve progredir quando TX estiver saturado. Prioridades e divisão entre os dois núcleos ESP32 seguem ARCH-FW; ordem de software não prova WCET nem imunidade à interferência.
...
| XRCE, MID 04 | Um único dono TTY/UART demultiplexa XRCE de CONFIG/DATA e entrega ou recebe payload por ponte UDP em loopback ao Agent; LENGTH uint16 delimita a mensagem; FIFO HOST preserva até 64 datagramas de 128 bytes em ordem, sem limpeza por CONFIG; caminho best effort | Qualificar saturação, fragmentação, executor e QoS na versão micro-ROS escolhida |
...
|---|---|---|---|
| `/daqc_setup` | Host → DAQC | `microhil_interfaces/DaqcSetup` | Confirma o estado já solicitado por CONFIG, seleciona perfil e aplica configuração ADC/PWM/frequência de aquisição fora de STREAMING |
...
`DaqcSetup.msg` contém `uint8 command`, `uint32 profile_id`, `uint8 apply_configuration`, `uint8 adc_resolution_bits`, `uint8[6] adc_attenuation`, `uint32[2] pwm_frequency_hz`, `uint8[2] pwm_resolution_bits` e `uint16 acquisition_frequency_hz`. `command` usa DISABLE=1, ENABLE=2 e STREAMING=3, coerente com CONFIG do ICD. CONFIG é a única autoridade para transições de estado no enlace crítico. A DAQC rejeita um `DaqcSetup.command` diferente do estado efetivo e usa o valor coincidente como confirmação ROS da configuração solicitada. `profile_id=1` identifica o perfil ESP32 compilado, com schema congelado e nomes de I/O definidos antes de STREAMING; não transporta mapa de pinos ou descritores variáveis no ciclo.

`apply_configuration` vale 0 ou 1. Com valor 1, a DAQC só aceita a configuração em DISABLE ou ENABLE; deve validá-la e aplicá-la fora do caminho crítico antes de confirmar STREAMING. `adc_attenuation` usa a ordem GPIO32/33/34/35/36/39 e os códigos 0 dB=0, 2,5 dB=1, 6 dB=2 e 11 dB=3. `pwm_frequency_hz` e `pwm_resolution_bits` usam a ordem GPIO18/19. `acquisition_frequency_hz` configura a tarefa periódica AI/DI e aceita 1…100 Hz; valor inicial de YAML novo é 100 Hz. Duty PWM é `float32` no DATA de atuação e não é parte deste setup.
...
Para o perfil ESP32, `adc` é obrigatório e contém `resolution_bits` e `attenuation`. `resolution_bits` é comum aos seis canais e aceita 9…12. `attenuation` contém exatamente `GPIO32_AI`, `GPIO33_AI`, `GPIO34_AI`, `GPIO35_AI`, `GPIO36_AI` e `GPIO39_AI`, cada qual com código 0…3. `pwm` também é obrigatório e contém exatamente `GPIO18_PWM` e `GPIO19_PWM`; cada canal contém `frequency_hz` positivo e `resolution_bits` em 1…20. A GUI escreve todos esses campos, inclusive quando o I/O correspondente não estiver mapeado à FMU, para que a configuração aplicada seja explícita e reproduzível. A validação do perfil limita valores estruturais no host; a DAQC permanece responsável por rejeitar uma combinação frequência/resolução não realizável no periférico.

O YAML é exclusivamente configuração e não registra valores operacionais de entradas virtuais. O carregador HOST atual materializa `version`, `profile.id`, `execution`, `acquisition`, `adc`, `pwm` e a sequência única `mappings`, resolve direção/nome/tipo com a FMU e constrói os schemas DAQC→host de 28 bytes e host→DAQC de 21 bytes. `acquisition.frequency_hz` é opcional para manter perfis v1 anteriores e assume 100 Hz quando ausente; a GUI sempre o grava explicitamente e o host rejeita valor fora de 1…100 Hz. No modo terminal, o YAML só é carregado após a FMU e, quando válido, fornece `step_size_s` e `stop_time_s`; Play exige que cada saída mapeada esteja selecionada para leitura. Os valores de ADC/PWM/aquisição permanecem no objeto de perfil para publicação em `DaqcSetup`. O codificador de atuação aplica a transformação inversa `raw=(valor_fmu-offset)/scale` e valida a faixa física depois da transformação: AO 0…3,3 V, PWM 0…1 e DO binário. Na primeira saída inválida ou fora da faixa sem histórico ele falha; após um valor físico válido, retém o último valor aceito para aquele canal. Um índice interno da lista de outputs da FMU, associado ao nome, tipo e `valueReference` resolvidos, preserva essa identidade depois da ordenação de apresentação/log. `DaqActuationZero` cria o payload de encerramento. A bridge HOST publica esse payload DATA apenas em STREAMING confirmado e o runner a chama depois de cada passo FMI.
