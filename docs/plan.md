...
| TASK-003 | Validação, FMU, retenção de inputs DAQC no host, proteção por passo e estados; F-01/05/06/07/08/09/21/23/24/25, NF-14 | TASK-001 para integração; DEC-011 para fixtures/suporte | Inputs inválidos rejeitados antes de threads, initialize/run/stop/restart/error e outputs iniciais/zero de encerramento testados, sem sucesso de zero passos por NaN | Em andamento: o runner inicializa a FMU e resolve os valores iniciais de input antes de criar a thread; a fixture confirma que essa preparação não chama `doStep` e que o run posterior executa passos fixos. Integração DAQC, transições externas, Stop/Error e HIL pendentes |
...
| TASK-007 | Implementação host de comunicação; F-18/19/20/23/24/27, NF-02/04/05/06/07/08/18/19/20/21/22/23/24/25/29 | ICD 0.22 e DEC-001/004/006/012 | Codec/parser, frames fragmentados/agregados, gap sem retransmissão, mailboxes, READ_ACK, ponte XRCE, TTY POSIX e testes de integração | Em andamento: codec/parser, MTU XRCE 128, TTY Linux com 152.000 bit/s, bridge de atuação para mailbox DATA, controlador ENABLE→STREAMING/STREAMING→DISABLE com prazo configurável de CONFIG, mailboxes XRCE RX/TX e ponte UDP de loopback verificados em HOST; Agent Humble UDP compilado/iniciado em ambiente temporário. O nó `rcl` do runner foi compilado e testado em thread própria para publicar setup e receber estado/erros; a ligação ao Play aguarda definição do prazo ROS. Sessão XRCE, conexão ao runner e enlace físico pendentes; worker TTY HOST verificado em pseudo-terminal |
