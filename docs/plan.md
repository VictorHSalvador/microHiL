...
O acompanhamento de implementação usa oito grupos derivados das dependências acima. Ele não substitui as TASKs nem altera requisitos: 1) controlador de execução compartilhado; 2) YAML e persistência na GUI; 3) seleção de saídas, mapeamento e entradas virtuais; 4) gráficos por saída; 5) Play/Stop, resultado, logging e CSV na GUI; 6) integração host–Agent–CH340; 7) firmware e bancada ESP32; 8) qualificação temporal/HIL e evidências finais. O grupo 1 está em verificação nesta revisão; os demais permanecem pendentes.
