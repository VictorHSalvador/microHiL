...
REQ-NF-26 recebido prescrevia variáveis camelCase e funções UPPER_CASE. O protótipo usa funções snake_case e tipos PascalCase. O usuário substituiu explicitamente a convenção: funções C PascalCase, variáveis snake_case, constantes UPPER_CASE e tipos próprios snake_case com `_t`. Também definiu Python conforme tabela PEP 8 de nomes, comentários em inglês de desenvolvedor e ausência de quebras cosméticas de parâmetros.
