...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo host-run-logging
set -u
set -o pipefail
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/host-run-logging/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/microHiL-host-run-logging-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/microHiL-host-run-logging"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/host-run-logging/executar-tarefas.sh --faixa $1"
...
T-005 — "Implementar e testar o codec binário tipado"
  critérios/refs: AC-015 (O cabeçalho identifica o formato, a FMU e as saídas), AC-016 (Cada passo possui um registro tipado e sem métricas de desempenho), AC-017 (Entradas malformadas são rejeitadas sem alocação ilimitada)
  arquivos permitidos (e seus testes): include/log_format.h, src/log_format.c, tests/test_log_format.c, tests/CMakeLists.txt, CMakeLists.txt
  mensagem de commit: "T-005 host-run-logging: Implementar e testar o codec binário tipado"
...
Sua tarefa (somente ela):
T-006 — "Implementar o logger assíncrono e os testes concorrentes"
  critérios/refs: AC-018 (O produtor nunca executa I/O do sink), AC-019 (O encerramento drena todas as amostras aceitas), AC-020 (Falhas do sink são visíveis e não encerram a simulação), AC-021 (A fila mantém ordem sob produtor e consumidor concorrentes), AC-026 (Diagnóstico próprio usa retorno estruturado)
  arquivos permitidos (e seus testes): include/binary_logger.h, src/binary_logger.c, tests/test_binary_logger.c, tests/test_sample_queue.c, tests/CMakeLists.txt, CMakeLists.txt
  mensagem de commit: "T-006 host-run-logging: Implementar o logger assíncrono e os testes concorrentes"
...
Sua tarefa (somente ela):
T-007 — "Implementar e testar a conversão pós-execução para CSV"
  critérios/refs: AC-022 (Conversão durante Running é recusada), AC-023 (Identidade e esquema incompatíveis impedem a conversão), AC-024 (O CSV preserva tipos, qualidade e registros completos)
  arquivos permitidos (e seus testes): include/log_converter.h, src/log_converter.c, tests/test_log_converter.c, tests/CMakeLists.txt, CMakeLists.txt
  mensagem de commit: "T-007 host-run-logging: Implementar e testar a conversão pós-execução para CSV"
...
Sua tarefa (somente ela):
T-008 — "Integrar logging binário ao runner e ao resultado final"
  critérios/refs: AC-018 (O produtor nunca executa I/O do sink), AC-019 (O encerramento drena todas as amostras aceitas), AC-020 (Falhas do sink são visíveis e não encerram a simulação), AC-025 (O resultado agregado não altera as métricas da simulação), AC-026 (Diagnóstico próprio usa retorno estruturado)
  arquivos permitidos (e seus testes): include/app_config.h, include/common.h, include/rt_simulation.h, src/app_config.c, src/main.c, src/rt_simulation.c, src/csv_logger.c, include/csv_logger.h, CMakeLists.txt, tests/test_run_logging.c, tests/CMakeLists.txt, test/run_spec_tests.js
  mensagem de commit: "T-008 host-run-logging: Integrar logging binário ao runner e ao resultado final"
...
Sua tarefa (somente ela):
T-009 — "Atualizar SDD, rastreabilidade e evidência da TASK-002"
  critérios/refs: AC-015 (O cabeçalho identifica o formato, a FMU e as saídas), AC-016 (Cada passo possui um registro tipado e sem métricas de desempenho), AC-017 (Entradas malformadas são rejeitadas sem alocação ilimitada), AC-018 (O produtor nunca executa I/O do sink), AC-019 (O encerramento drena todas as amostras aceitas), AC-020 (Falhas do sink são visíveis e não encerram a simulação), AC-021 (A fila mantém ordem sob produtor e consumidor concorrentes), AC-022 (Conversão durante Running é recusada), AC-023 (Identidade e esquema incompatíveis impedem a conversão), AC-024 (O CSV preserva tipos, qualidade e registros completos), AC-025 (O resultado agregado não altera as métricas da simulação), AC-026 (Diagnóstico próprio usa retorno estruturado)
  arquivos permitidos (e seus testes): README.md, docs/README.md, docs/contracts/interfaces.md, docs/plan.md, docs/tasks/TASK-002.md, docs/traceability.md, docs/quality-gates.md, docs/sdd-versions.md, docs/evidence/host-run-logging-2026-09-07.md, .spec/features/host-run-logging/spec.md, .spec/features/host-run-logging/tasks.md, test/sdd-versioning.spec.test.js, onpspec.config.json
  mensagem de commit: "T-009 host-run-logging: Atualizar SDD, rastreabilidade e evidência da TASK-002"
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/host-run-logging/executar-tarefas.sh --gate"
      exit 0
    fi
