...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo host-build-foundation
set -u
set -o pipefail
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/host-build-foundation/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/microHiL-host-build-foundation-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/microHiL-host-build-foundation"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/host-build-foundation/executar-tarefas.sh --faixa $1"
...
executar_seq_T_004() {
  ...
  if rodar_tarefa seq 'T-004' 'Você executa UMA tarefa da feature "host-build-foundation" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/host-build-foundation/spec.md, .spec/features/host-build-foundation/tasks.md e .spec/constituicao.md.
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' high >> "$LOG_DIR/seq.log" 2>&1; then
    # commit de segurança se o agente esqueceu (rastreabilidade > perfeição)
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-004 host-build-foundation: Documentar, evidenciar e rastrear a TASK-001 (auto-commit do plano)'
    fi
  ...
  amarelo "  reexecute só ela: bash .spec/features/host-build-foundation/executar-tarefas.sh --seq T-004"
  FALHAS="$FALHAS T-004"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/host-build-foundation/executar-tarefas.sh --gate"
      exit 0
    fi
