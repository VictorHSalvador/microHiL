...
1. branch de trabalho `spec/host-build-foundation` criada do ponto atual (se ainda não existir)
2. as tarefas rodam nela mesma, na ordem — **1 tarefa = 1 commit** (`T-xxx feature: título`), marcada `[concluida]` só com trabalho feito
3. gate final na branch de trabalho: `onp-spec verify host-build-foundation` + `onp-spec audit --ci` — **exit 0 ou não está pronto**
...
```bash
bash .spec/features/host-build-foundation/executar-tarefas.sh
```
...
Logs: `../onp-worktrees/microHiL-host-build-foundation-logs/`.

**Confirmação de custos — antes de executar**: os modelos e esforços por
tarefa estão nas tabelas acima; o agente CONFIRMA com o usuário se estão
dentro da licença/cota dele (modelo forte + esforço alto torra tokens).
Para gastar menos: `onp-spec plano host-build-foundation --modelo gpt-5.6-luna --esforco baixo`
(tudo) ou por tarefa `onp-spec tarefa host-build-foundation T-xxx --modelo <m> --esforco <nível>` — e regenere o plano.
...
```bash
onp-spec resumo host-build-foundation --tabela   # a tabela de andamento
onp-spec resumo host-build-foundation            # o resumo em texto
```
