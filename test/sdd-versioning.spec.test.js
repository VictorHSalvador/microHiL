...
RegisterTest('AC-001: A versão vigente é identificável @spec:AC-001', () => {
  ...
  assert.match(versioning, /Versão vigente: \*\*SDD-MICROHIL 0\.26\.7\*\*, de 13\.09\.2026/);
...
RegisterTest('P-003: Estado documental não é estado de implementação @principle:P-003', () => {
  const versioning = Read('docs/sdd-versions.md');
  assert.match(versioning, /a hipótese de pilha\/execução síncrona micro-ROS recebeu correção diagnóstica, ainda sem gravação ou ensaio CONFIG desta imagem; validação do produto permanece pendente/);
