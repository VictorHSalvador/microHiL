...
RegisterTest('AC-001: A versão vigente é identificável @spec:AC-001', () => {
  ...
  assert.match(versioning, /Versão vigente: \*\*SDD-MICROHIL 0\.25\.11\*\*, de 12\.09\.2026/);
...
RegisterTest('AC-002: O estado separa especificação, implementação e verificação @spec:AC-002', () => {
  ...
  assert.match(versioning, /Não executa sessão XRCE, tópicos ROS, perfil aplicado, DATA, I\/O, Raspberry Pi, HIL ou timing real/i);
});
...
RegisterTest('P-003: Estado documental não é estado de implementação @principle:P-003', () => {
  const versioning = Read('docs/sdd-versions.md');
  assert.match(versioning, /a DAQC foi gravada e respondeu ao CONFIG seguro pela CH340; o handshake XRCE físico permanece pendente; validação do produto permanece pendente/);
