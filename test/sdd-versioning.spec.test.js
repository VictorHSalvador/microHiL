...
RegisterTest('AC-001: A versão vigente é identificável @spec:AC-001', () => {
  ...
  assert.match(versioning, /Versão vigente: \*\*SDD-MICROHIL 0\.29\.5\*\*, de 14\.09\.2026/);
...
RegisterTest('P-003: Estado documental não é estado de implementação @principle:P-003', () => {
  const versioning = Read('docs/sdd-versions.md');
  assert.match(versioning, /o runner C completou 200 passos físicos com Agent, DAQC e FMU; a GUI executou Play HiL físico por 500 passos com loopbacks digital, DAC\/ADC e PWM\/ADC observados no log e reflete o YAML carregado; precisão\/carga, duty intermediário PWM, Raspberry Pi e qualificação temporal permanecem pendentes/);
