...
RegisterTest('AC-001: A versão vigente é identificável @spec:AC-001', () => {
  ...
  assert.match(versioning, /Versão vigente: \*\*SDD-MICROHIL 0\.26\.11\*\*, de 13\.09\.2026/);
...
RegisterTest('P-003: Estado documental não é estado de implementação @principle:P-003', () => {
  const versioning = Read('docs/sdd-versions.md');
  assert.match(versioning, /o ciclo completo de transição de estados \(DISABLE→ENABLE→STREAMING→DISABLE\), aquisição periódica a ~96,4 Hz e confirmação cumulativa READ_ACK foram comprovados fisicamente na placa ESP32; validação com coordenador C permanece pendente/);
