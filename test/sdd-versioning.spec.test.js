...
RegisterTest('AC-001: A versão vigente é identificável @spec:AC-001', () => {
  ...
  assert.match(versioning, /Versão vigente: \*\*SDD-MICROHIL 0\.26\.6\*\*, de 13\.09\.2026/);
...
RegisterTest('P-003: Estado documental não é estado de implementação @principle:P-003', () => {
  const versioning = Read('docs/sdd-versions.md');
  assert.match(versioning, /a imagem da correção XRCE foi gravada com hash confirmado, mas o ensaio CONFIG seguro posterior não respondeu; validação do produto permanece pendente/);
