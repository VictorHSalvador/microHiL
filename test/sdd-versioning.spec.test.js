...
RegisterTest('AC-001: A versão vigente é identificável @spec:AC-001', () => {
  ...
  assert.match(versioning, /Versão vigente: \*\*SDD-MICROHIL 0\.29\.9\*\*, de 19\.09\.2026/);
...
RegisterTest('AC-003: Cada revisão possui metadados mínimos @spec:AC-003', () => {
  ...
  assert.equal(rows.at(-1)[0], '0.29.9');
  rows.pop();
  assert.equal(rows.at(-1)[0], '0.29.8');
  rows.pop();
  assert.equal(rows.at(-1)[0], '0.29.7');
  rows.pop();
  assert.equal(rows.at(-1)[0], '0.29.6');
  rows.pop();
  assert.equal(rows.at(-1)[0], '0.29.5');
  rows.pop();
  ...
  rows.pop();
  assert.equal(rows.at(-1)[0], '0.29.1');
  rows.pop();
  ...
  rows.pop();
  assert.equal(rows.at(-1)[0], '0.27.1');
  rows.pop();
  assert.equal(rows.at(-1)[0], '0.27.0');
  rows.pop();
  assert.equal(rows.at(-1)[0], '0.26.18');
  rows.pop();
  ...
  rows.pop();
  assert.equal(rows.at(-1)[0], '0.26.14');
  rows.pop();
  assert.equal(rows.at(-1)[0], '0.26.13');
  rows.pop();
  assert.equal(rows.at(-1)[0], '0.26.12');
  rows.pop();
  assert.equal(rows.at(-1)[0], '0.26.11');
  rows.pop();
...
RegisterTest('P-003: Estado documental não é estado de implementação @principle:P-003', () => {
  const versioning = Read('docs/sdd-versions.md');
  assert.match(versioning, /o runner C completou 200 passos físicos com Agent, DAQC e FMU; a GUI executou Play HiL físico por 500 passos com loopbacks digital, DAC\/ADC e PWM\/ADC observados no log, reflete o YAML carregado e normaliza URLs locais do seletor de arquivos; precisão\/carga, duty intermediário PWM, Raspberry Pi e qualificação temporal permanecem pendentes/);
